...
def get_last_four_weeks(user):
    ...
    for number_week in range(1, 5):
        ...
        week_date = get_period_week(added_date=day_of_week)
...
def get_statistic(user):
    """Return worked time in hours for:
    
    current month, last month, ever time
    """
    month = date.today().month

    statistic = {
        'current_month': get_worked_time(user, ),
        'last_month': get_worked_time(user, month-1),
